...
const routes=[['home','home','首页'],['transactions','transactions','账单'],['analysis','analysis','分析'],['pending','pending','待处理'],['recycle','recycle','回收站'],['settings','settings','设置']] as const;
let route='home',shared=false,revision=0;
document.querySelector('#app')!.innerHTML=`<aside class="sidebar"><a href="#home" class="brand" aria-label="轻账首页"><span class="brand-icon">${icon('ledger')}</span><span>轻账<small>LIGHT LEDGER</small></span></a><div class="sidebar-caption">我的日常</div><nav aria-label="主导航">${routes.map(([id,glyph,label])=>`<button data-route="${id}" aria-label="${label}">${icon(glyph)}<span class="nav-label">${label}</span>${id==='pending'?'<span id="pending-count" class="count" hidden></span>':''}</button>`).join('')}</nav><div class="sidebar-bottom"><span class="local-dot">${icon('status')}</span> 本地优先 · 安心记录<small>让生活有数，让日子轻一点。</small></div></aside><div class="workspace"><header class="topbar"><div class="breadcrumb">我的账本 <span>/</span> <strong id="route-name">首页</strong></div><div class="scope" role="group" aria-label="账本范围"><button id="mine" aria-pressed="true">我的</button><button id="ours" aria-pressed="false">我们</button></div><button class="primary with-icon" id="quick-add">${icon('plus')} 记一笔</button></header>${desktop?'':`<div class="preview-banner">${connected?'开发验收 · 已连接临时账本，系统能力需要在本机应用中使用。':'界面预览 · 未连接本地账本，请在轻账应用中操作。'}</div>`}<main id="main"></main><div id="status" role="status" aria-live="polite" hidden></div><footer class="app-footer"><span>轻账 · 为每一份小日常</span><span>${desktop?'本机账本':'浏览器界面预览'}</span></footer></div>`;
...
async function navigate(next:string){route=next;const revisionNow=++revision;document.querySelector('#route-name')!.textContent=routes.find(r=>r[0]===route)![2];document.querySelectorAll<HTMLElement>('[data-route]').forEach(b=>{b.classList.toggle('active',b.dataset.route===route);b.setAttribute('aria-current',b.dataset.route===route?'page':'false');});
 ...
 try{
  if(route==='home'||route==='analysis'){
   ...
   if(route==='home'){
    main.innerHTML=heading(shared?'一起，把日子过清楚。':'把日子，记得轻一点。','每一笔小小的记录，都是认真生活的证据。')+`<section class="assistant-card"><div class="assistant-icon" aria-hidden="true">${icon('sparkles')}</div><div><div class="assistant-label">你的记账小助手 <span>本地账本</span></div><h2>${pendingItems.length?`有 ${pendingItems.length} 件事，等你确认。`:'今天，也从容一点。'}</h2><p>${summaries.length?`账本已经记录 ${summaries.reduce((n,s)=>n+s.count,0)} 笔日常，随时回来看看。`:'还没有账单。记下第一笔，或导入已有的账单文件。'}</p><div class="actions"><button class="primary with-icon" id="natural">说一句，记一笔 ${icon('arrowUpRight')}</button><button id="import">导入账单</button><button id="import-image">导入截图</button></div></div></section><div class="stats">${summaryCards(summaries)}</div><section class="card recent"><div class="section-heading"><div><h2>最近的日常</h2><p>小事值得被记住。</p></div><button class="text-button with-icon" id="all-transactions">查看全部 ${icon('arrowRight')}</button></div><div id="list"></div></section>`;
