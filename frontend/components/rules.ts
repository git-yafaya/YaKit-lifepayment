...
export async function rules(host:HTMLElement){
 ...
 host.innerHTML=`<div class="section-heading"><div><h2>分类规则</h2><p>仅在你确认后，用商户名称补充分类。</p></div><button id="add-rule">添加规则</button></div>${values.map(r=>`<div class="setting-row"><span>${escape(r.merchant)} → ${escape(r.category)} · ${r.enabled?'已启用':'已暂停'}</span><button data-rule="${escape(r.id)}">撤销规则</button></div>`).join('')||'<p class="muted">还没有分类规则。</p>'}`;
